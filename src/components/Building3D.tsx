...
// Slot ve balkon pozisyonları buildScene içinde bw'e göre dinamik hesaplanıyor
...
const Building3D: React.FC<Building3DProps> = ({
    ...
    const buildScene = useCallback((scene: THREE.Scene): number => {
        ...
        // Gerçek ölçüler (PDF'den geldiyse kullan, yoksa default)
        const bw = buildingWidth  ?? BW;
        const bd = buildingDepth  ?? BD;

        // Slot pozisyonlarını bw'e göre hesapla
        const slots = [
            { x: -bw * 0.22, side:  1 },
            { x:  bw * 0.22, side:  1 },
            { x:  bw * 0.22, side: -1 },
        ...
        floors.forEach((floor, fi) => {
            ...
            apts.forEach((apt, ai) => {
                ...
                const _dim  = apt.status === 'sold' || apt.status === 'common'; void _dim;

                // Zemin katta pencere değil vitrin var, diğer katlarda pencereler
                if (floor !== 0) {
                    ...
                    WIN_DX.forEach((dx, wi) => {
                        ...
                        // Koyu navy çerçeve (duvardan hafif çıkıntılı)
                        ...
                        // Sadece orta pencere tıklanabilir
                        if (wi === 1) {
                            glass.userData = { apt };
                    ...
                    // Daire no etiketi
                    const label = apt.apartment_number
                        ? `D.${apt.apartment_number}`
                        : `D.${ai + 1}`;
                ...
                if (floor >= 1) {
                    ...
                    // Dubleks — üst seviye balkon (daha küçük)
                    if (duplex) {
        ...
        // ── Çatı — geniş navy saçak (renderlarla eşleşiyor) ─────────────
        const roofH   = 0.55;
